...
type
  ...
  private
    ...
    procedure SetImageList(const Value: FXImageList);
  ...
  published
    ...
    property SelectImageList: FXImageList read FImageList write SetImageList;
...
procedure FXIconSelect.AssignTo(Dest: TPersistent);
begin
  if Dest is FXIconSelect then begin
    const Destination = FXIconSelect(Dest);

    Destination.FType := Self.FType;

    Destination.FPicture.Assign(Self.FPicture);
    Destination.FBitMap.Assign(Self.FBitMap);
    Destination.FSegoeText := Self.FSegoeText;
    Destination.FImageIndex := Self.FImageIndex;
    Destination.FImageList := Self.FImageList;
...
begin
  case IconType of
    ...
    FXIconType.ImageList: if Assigned(FImageList) and InRange(FImageIndex, 0, FImageList.Pictures.Count-1) then begin
      const Img = FImageList.Pictures[FImageIndex].Graphic;
      if Img <> nil then
        try
          if not Img.Empty then
            DrawImageInRect( Canvas, ARectangle, Img, TDrawMode.CenterFit );
        except
        end;
    end;
...
procedure FXIconSelect.SetImageList(const Value: FXImageList);
begin
  FImageList := Value; // set, do NOT assign
